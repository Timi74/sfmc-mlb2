...
router.get('/:package([-\\w]+)/:mid([-\\w]+)/', async function (req, res) {
	try {
		...
		let sfmc_conf = {clientId: packageData.apiClientId, clientSecret: packageData.apiClientSecret, authBaseUrl:packageData.appUrl, mid: req.params.mid, scope: "data_extensions_read data_extensions_write documents_and_images_read" };
